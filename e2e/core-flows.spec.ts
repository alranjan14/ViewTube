...
test.describe('Core Navigation Flows', () => {
  ...
  test('should navigate to search results', async ({ page }) => {
    ...
    await expect(page).toHaveURL(
      /\/results\?search_query=Playwright(%20|\+)test/
    );
  });

  test('should navigate to the library page via sidebar', async ({
    page,
    isMobile,
  }) => {
    if (isMobile) {
    ...
    // The library page renders its sections (History / Watch Later) as h2 headings.
    await expect(
      page.getByRole('heading', { name: 'History', level: 2 })
    ).toBeVisible();
  });
});
