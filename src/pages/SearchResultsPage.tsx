...
const SearchResultsPage = () => {
  ...
  const {
    ...
    hasNextPage,
    isFetchingNextPage,
  } = useSearchVideos(query, 25, filters);
        ...
        <div className="flex flex-col sm:flex-row gap-8 mb-8 p-4 bg-slate-50 border border-slate-200 rounded-2xl">
          <div className="flex flex-col gap-3">
            ...
            <button
              ...
              className={`text-left text-sm font-medium ${!filters.publishedAfter ? 'text-slate-900' : 'text-slate-500 hover:text-slate-800'}`}
            >
              Any time
            </button>
            <button
              onClick={() => setTimeFilter('today')}
              className={`text-left text-sm font-medium ${filters.publishedAfter === getPublishedAfter('today') ? 'text-slate-900' : 'text-slate-500 hover:text-slate-800'}`}
            >
              Today
            </button>
            <button
              onClick={() => setTimeFilter('week')}
              className={`text-left text-sm font-medium ${filters.publishedAfter === getPublishedAfter('week') ? 'text-slate-900' : 'text-slate-500 hover:text-slate-800'}`}
            >
              This week
            </button>
            <button
              onClick={() => setTimeFilter('month')}
              className={`text-left text-sm font-medium ${filters.publishedAfter === getPublishedAfter('month') ? 'text-slate-900' : 'text-slate-500 hover:text-slate-800'}`}
            >
              This month
            </button>
          </div>
          <div className="flex flex-col gap-3">
            ...
            <button
              onClick={() => setFilters((p) => ({ ...p, order: 'relevance' }))}
              className={`text-left text-sm font-medium ${!filters.order || filters.order === 'relevance' ? 'text-slate-900' : 'text-slate-500 hover:text-slate-800'}`}
            >
              Relevance
            </button>
            <button
              onClick={() => setFilters((p) => ({ ...p, order: 'date' }))}
              className={`text-left text-sm font-medium ${filters.order === 'date' ? 'text-slate-900' : 'text-slate-500 hover:text-slate-800'}`}
            >
              Upload date
            </button>
            <button
              onClick={() => setFilters((p) => ({ ...p, order: 'viewCount' }))}
              className={`text-left text-sm font-medium ${filters.order === 'viewCount' ? 'text-slate-900' : 'text-slate-500 hover:text-slate-800'}`}
            >
              View count
            </button>
          </div>
        ...
        {(isLoading || isFetchingNextPage) && (
          <div className="flex flex-col gap-4 mt-2">
            {Array.from({ length: 5 }).map((_, i) => (
              <div
                key={i}
                className="flex flex-col sm:flex-row gap-4 p-2 w-full"
              >
