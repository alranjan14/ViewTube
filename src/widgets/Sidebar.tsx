...
const SidebarItem = ({
  ...
  return (
    ...
    >
      <Icon
        size={isMenuOpen ? 22 : 24}
        strokeWidth={isActive ? 2.5 : 2}
        className={`flex-shrink-0 ${isActive ? 'text-slate-900' : 'text-slate-700 group-hover:text-slate-900'}`}
      />
      <span
        className={`truncate text-slate-900 ${isMenuOpen ? 'text-sm' : 'text-[10px]'}`}
      >
        {label}
      </span>
    </Link>
...
const Sidebar = () => {
  ...
  return (
    <aside
      ...
      className={`sticky top-16 h-[calc(100vh-64px)] overflow-y-auto custom-scrollbar flex-shrink-0 hidden sm:block border-r border-slate-200/50 bg-white/50 backdrop-blur-md transition-all duration-300 ${isMenuOpen ? 'w-64 px-3' : 'w-20 px-1'}`}
    >
      ...
      {isMenuOpen && (
        <>
          ...
          <div className="py-3 flex flex-col gap-1">
            <h3 className="px-4 text-base font-semibold mb-1 text-slate-900">
              You
            </h3>
            <SidebarItem
              icon={History}
          ...
          <div className="px-4 py-4 mt-auto">
            ...
            <p className="text-[10px] text-slate-400 mt-4 leading-relaxed">
              Data provided by YouTube API. Not affiliated with YouTube or
              Google LLC.
            </p>
